...
class Embedder(Protocol):
    ...
    Живой GigaChat-эмбеддер приходит в S7 с мозгами агентов (spec/70); для CI и юнитов —
...
def embedder(provider: str, model: str) -> Embedder:
    """Фабрика эмбеддера по провайдеру из чертежа. Прод-путь: дубля здесь нет (spec/05, spec/70).

    gigachat — честный отказ до S7; иных провайдеров прод не знает. Тесты подают свою фабрику
    с детерминированным дублём — так дубль не пробивается в прод-путь.
    """
    if provider == "gigachat":
        raise NotImplementedError(
            "живой GigaChat-эмбеддер приходит в S7 вместе с мозгами агентов (spec/70); "
            "в S5 инжест гоняется на детерминированном дубле из tests/"
        )
    raise ValueError(
        f"неизвестный провайдер эмбеддера '{provider}' (модель '{model}') — "
        f"прод знает только gigachat (приходит в S7)"
    )
