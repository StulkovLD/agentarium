...
- **Агент падает.** Это штатно: конверт ждёт в очереди, docker перезапустит, ретраи в SDK. Итог заявки — в `GET /requests/{id}` (status failed); в Jaeger виден путь до `task.failed`.
